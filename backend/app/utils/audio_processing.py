...
def export_silero_segments(segments: List[Dict], original_audio: Any, sample_rate: int, 
                          output_dir: str = "silero_segments", min_duration: float = 0.5, 
    ...
    for i, segment in enumerate(segments):
        ...
        if end_sample > start_sample:
            ...
            print(f"💾 导出段 {i+1:04d}: {start_time:.2f}s - {end_time:.2f}s "
                  f"(时长: {duration:.2f}s)")
...
def parse_transcription_segments(transcription_lines: List[str], segment_start_time: float, 
                                segment_end_time: float) -> List[Dict]:
    """
    解析转录结果，使用VAD分段的时间戳
