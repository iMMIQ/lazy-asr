...
import { Download, FileText, CheckCircle2, XCircle, Clock, Database, AlertCircle, LucideIcon } from 'lucide-react';

/** Segment information from transcription result */
...
/** File result in batch processing */
...
export interface ResultDisplayProps {
  result?: SingleFileResult | null;
  multiFileResult?: MultiFileResult | null;
  onDownload?: (filePath: string) => void;
  onBundleDownload?: (taskId: string) => void;
}
...
export function ResultDisplay({
  ...
  const StatItem = ({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: number | string }) => (
    <li>
  ...
  const handleBundleDownload = (taskId: string) => {
    if (onBundleDownload) {
      onBundleDownload(taskId);
      return;
    }
