...
const ConfigPanel = ({
    ...
    const handleVadChange = (field, value) => {
        if (onVadConfigChange) {
            onVadConfigChange(field, parseInt(value) || (field === 'minSpeechDuration' ? 500 : 500));
        }
    };
            ...
            <div className="form-group">
                ...
                <div className="format-checkboxes">
                    {OUTPUT_FORMATS.map((format) => (
                        <label
                            key={format}
                            className={`format-checkbox ${outputFormats && outputFormats.includes(format) ? 'selected' : ''}`}
                        >
                            <input
                                type="checkbox"
                                checked={outputFormats && outputFormats.includes(format)}
